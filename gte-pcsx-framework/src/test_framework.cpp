...
void TestFramework::print_detailed_report() const {
    ...
    for (const auto& result : results_) {
        ...
        std::cout << "  " << status << " " << result.test_name << " (cmd: 0x" << std::hex << std::setw(2) << std::setfill('0') << (result.command & 0x3F) << " op: 0x" << std::setw(8) << std::setfill('0') << (result.raw_opcode ? result.raw_opcode : result.command) << std::dec << ")\n";
